...
def creds_object(request,hit_db = False):
    if hit_db:
        ...
        creds_f = Credentials(
            ...
            expiry=record.expiry.replace(tzinfo=None) if record.expiry else None,
        )
...
def list_of_bills(request,UUIDS,start_date=None,end_date=None):
    ...
    l,l1=[],[]
    for UUID in UUIDS:
        folders = folder_check_create(service,folder_id,[return_cat_name(request, UUID)])
        cat_id = folders[return_cat_name(request, UUID)]
